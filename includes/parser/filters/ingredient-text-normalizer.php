 ...
 * Ziel:
 *   (a) Komma-Listen nicht zu Notes einklammern
 *   (b) Ampersand/und: nie LHS verlieren
 *   (c) Leit-Substantive wie "Saft" nicht strippen
 *   (d) Pseudo-Unicode wie "u00f6" → "ö" decodieren
 *
 * Einbindung (vor QTY/Unit):
...
final class Ingredient_Text_Normalizer
{
    ...
    public static function normalize(string $line): string
    {
        ...
        // 0) Pseudo-Unicode wie "u00f6" → "ö" und echte JSON-Escapes "\u00f6" → "ö"
        $line = self::decodePseudoUnicode($line);
    ...
    {
        // Zero-Width & Whitespace normalisieren
    ...
    /**
     * "u00f6", "u00D6" → "ö", "Ö"
     * Auch echte JSON-Escapes "\u00f6" → "ö" abfangen.
     */
    private static function decodePseudoUnicode(string $t): string
    {
        if ($t === '') return $t;

        // Fall A: echte JSON-Escapes wie "\u00f6"
        if (strpos($t, '\u00') !== false) {
            $json = '"' . str_replace(['\\', '"'], ['\\\\', '\"'], $t) . '"';
        ...
        // Fall B: pseudo "u00f6" ohne Backslash (aus manchen Quellen)
        // Ersetze beliebige u00XX-Sequenzen
        $t = preg_replace_callback('/u00([0-9a-fA-F]{2})/u', static function ($m) {
            $hex = $m[1];
            $code = hexdec($hex);
            // Latin-1 → UTF-8
            return iconv('ISO-8859-1', 'UTF-8//IGNORE', chr($code));
        }, $t) ?? $t;
    ...
    /**
     * Fix: „Salz & Pfeffer“ darf nie zu „& Pfeffer“ werden.
     */
    ...
    /**
     * Komma-Listen werden fälschlich zu "(...)" Note umgeformt.
     * Regel:
     *  - Erlaube Komma->Note NUR, wenn der nachgestellte Teil klare Note-Hints enthält.
     */
    ...
    /** Head-Nouns wie "Saft", "Schale", "Abrieb" nicht entfernen. */
