...
function sitc_qty_pre_normalize_v2(string $s): string {
    ...
    $t = preg_replace('/^(ca\.?|circa|etwa|ungef(?:ae|ä)hr\.?|about|approx\.?|approximately)[\s\(\)]*/iu', '', $t);
...
function sitc_extract_leading_qty_token_v2(string $line): string {
    ...
    // capture token(s) like: 1, 1/2, 1 1/2, 1-2, 1,5, ½, 1½, etc. at the start
...
function sitc_coerce_qty_float_v2($qty) {
    ...
    // If range like a-b or a–b, use low bound
